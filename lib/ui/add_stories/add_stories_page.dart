...
class AddStoriesPage extends GetView<AddStoriesController> {
  AddStoriesPage({Key? key}) : super(key: key);
                  ...
                  child: SingleChildScrollView(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        FormFieldText(controller: _modelUrl, hint: 'Modelo 3D', iconData: Icons.emoji_people),
                        //form field story
                        Padding(
                          ...
                          child: Obx(() {
                            return (controller.isLoading)
                                ...
                                : SizedBox(
                                    ...
                                    width: double.infinity,
                                    child: (controller.portraitPath == './././assets/images/default_image.png')
                                        ? Row(
                                            mainAxisAlignment: MainAxisAlignment.spaceEvenly,
                                            children: [
                                              CustomButton(
  ...
  positionMissing() {
    Get.dialog(
      AlertDialog(title: const Text('Error'), content: const Text('La ubicación de la historia esta vacía.'), actions: [
        OutlinedButton(
            onPressed: () {
              Get.back();
            },
  ...
  portraitMissing() {
    Get.dialog(
      AlertDialog(
        ...
        content: const Text('La portada de la historia esta vacía.'),
        actions: [
          OutlinedButton(
            onPressed: () {
              Get.back();
  ...
  saveData() async {
    log('lat lng is ${storiesMapController.newMarker.elementAt(0).position}');
    GeoPoint geoPoint = GeoPoint(storiesMapController.newMarker.elementAt(0).position.latitude,
    ...
    final story = Story(
        ...
        imageUrl: controller.portraitPath,
        active: (controller.selectedActive == 'Activado') ? true : false,
        type: (controller.selectedType == 'Mito') ? 'myth' : 'legend',
        modelUrl: _modelUrl.text);
    log(story.toString());
    bool saved = await controller.saveStory(story);
    if (saved) {
      ...
      storiesMapController.resetMarker();
      Get.dialog(AlertDialog(
        ...
        actions: [
          OutlinedButton(
            ...
            child: const Text('Historia agregada correctamente.'),
          ),
        ],
      ));
    } else {
      Get.dialog(AlertDialog(
        ...
        actions: [
          OutlinedButton(
            ...
            child: const Text('La historia no ha sido agregada.'),
          ),
        ],
      ));
    }
